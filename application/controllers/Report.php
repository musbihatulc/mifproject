...
class Report extends CI_Controller {
    ...
    public function index() {
        ...
        if($jenis == null || $tahun == null) {
            $data['report'] = $this->db->query("SELECT * FROM `tm_user`")->result();
        } else if ($jenis == 'Aktif' && $tahun != 'All'){
            $data['report'] = $this->db->query("SELECT * FROM `tm_user` WHERE status_mutasi='$jenis' AND tgl_mutasi BETWEEN DATE(NOW()) - INTERVAL '$tahun' YEAR AND DATE(NOW())")->result();
        } else if ($jenis == 'Tidak Aktif' && $tahun != 'All'){
            $data['report'] = $this->db->query("SELECT * FROM `tm_user` WHERE status_mutasi='$jenis' AND tgl_masuk BETWEEN DATE(NOW()) - INTERVAL '$tahun' YEAR AND DATE(NOW())")->result();
        } else if ($tahun == 'All') {
            $data['report'] = $this->db->query("SELECT * FROM `tm_user` WHERE status_mutasi='$jenis'")->result();
        } 
